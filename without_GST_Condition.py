...
def calculation():
    ...
    finaltotal = float('%.2f' % (sub + cgst + sgst))
...
def generate_invoice():
    if old_list == []:
        ...
        if (w == "yes"):
            doc = DocxTemplate('Invoice Template/invoice1.docx')
            s = datetime.datetime.now()
            y = s.date().strftime('%d/%m/%y')
            p = s.date().strftime('%d_%m_%y_')
...
e7 = tkinter.Spinbox(frame, from_=1, to=100)
